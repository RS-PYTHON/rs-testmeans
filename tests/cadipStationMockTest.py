...
def test_querry_sessions(client, session_response20230216, login):
    """Docstring to be added."""
    ...
    # test without args
    assert client.get("Sessions", headers=auth_header).status_code == BAD_REQUEST
    # test with an incorrect filter
    assert client.get("Sessions?$filter=Incorrect_filter", headers=auth_header).status_code == BAD_REQUEST
    # Response containing more than 1 result, since there are more products matching
    response = client.get("Sessions?$filter=PublicationDate gt 2019", headers=auth_header)
    assert len(json.loads(response.text)["responses"]) > 1
    # Response containing exactly one item, since explicit date is mentioned.
    response = client.get("Sessions?$filter=PublicationDate eq 2023-02-16", headers=auth_header)
    ...
    response = client.get("Sessions?$filter=PublicationDate lt 1999", headers=auth_header)
    assert not response.text
    # Test with sattelite - pos
    # Test status code - 200 OK, test that reponse exists and it's not empty
    assert client.get("Sessions?$filter=Satellite eq S1A", headers=auth_header).status_code == OK
    assert len(client.get("Sessions?$filter=Satellite eq 'S1A'", headers=auth_header).get_data())
    # Test with sattelite - neg
    # Test status code - 200 OK, test that reponse is empty as per ICD
    assert client.get("Sessions?$filter=Satellite eq INCORRECT", headers=auth_header).status_code == OK
    assert not client.get("Sessions?$filter=Satellite eq INCORRECT", headers=auth_header).get_data()
    # Test with Downlink - pos - status 200 and valid content
    assert client.get("Sessions?$filter=DownlinkOrbit eq 62343", headers=auth_header).status_code == OK
    assert len(client.get("Sessions?$filter=DownlinkOrbit eq 62343", headers=auth_header).get_data())
    # Test with Downlink - neg - status 200 and invalid content
    assert client.get("Sessions?$filter=DownlinkOrbit eq INCORRECT", headers=auth_header).status_code == OK
    assert not client.get("Sessions?$filter=DownlinkOrbit eq INCORRECT", headers=auth_header).get_data()
...
def test_download_file(client, original_path, download_path, original_file, download_file, login):
    """Docstring to be added."""
    ...
    assert client.get(api_route, headers=auth_header).status_code == NOT_FOUND
    # Test existing file
    api_route = "Files(some_id_2)/$value"
    response = client.get(api_route, headers=auth_header)
