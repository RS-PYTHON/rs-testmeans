...
def test_querry_products(client, products_response, login):
    """Docstring to be added."""
    ...
    # test without args
    assert client.get("Products", headers=auth_header).status_code == BAD_REQUEST
    # test with an incorrect filter
    assert client.get("Products?$filter=Incorrect_filter", headers=auth_header).status_code == BAD_REQUEST
    # Response containing more than 1 result, since there are more products matching
    response = client.get("Products?$filter=PublicationDate gt 2019", headers=auth_header)
    assert len(json.loads(response.text)["responses"]) > 1
    # Response containing exactly one item, since explicit date is mentioned.
    response = client.get("Products?$filter=PublicationDate eq 2023-02-16", headers=auth_header)
    assert isinstance(json.loads(response.text), dict)
    # Check response content with test-defined one.
    response = client.get("Products?$filter=PublicationDate eq 2019-02-16", headers=auth_header)
    assert json.loads(response.text).keys() == products_response.keys()
    assert json.loads(response.text) == products_response
    # Name contains.
    response = client.get("Products?$filter=contains(Name, S2__OPER_AUX_ECMWFD_PDMC_20190216T120)", headers=auth_header)
    assert json.loads(response.text).keys() == products_response.keys()
    assert json.loads(response.text) == products_response
    # Name contains enclosed with ''.
    response = client.get("Products?$filter=contains(Name, 'S2__OPER_AUX_ECMWFD_PDMC_20190216T1')", headers=auth_header)
    assert json.loads(response.text).keys() == products_response.keys()
    assert json.loads(response.text) == products_response
    # name startwith
    response = client.get("Products?$filter=startswith(Name, S2__OPER_AUX_ECMWFD_PDMC_2019)", headers=auth_header)
    assert json.loads(response.text).keys() == products_response.keys()
    assert json.loads(response.text) == products_response
    # Empty json response since there are no products older than 1999.
    response = client.get("Products?$filter=PublicationDate lt 1999", headers=auth_header)
