...
class single_unit_mockup(EOProcessingUnit):  # pylint: disable=invalid-name
    ...
    def run(self, inputs, adfs=None, mode=None, **kwargs):  # pylint: disable=unused-argument
        ...
        output_product_types = self.output_product_types(kwargs.get("output_products"))
        fixture_paths = self.fixture_paths()
        logger.info(f"Running DPR EOPF mockup processing unit step={self.identifier}")
        logger.info(f"Generating mock products for output types {output_product_types} from {len(fixture_paths)} fixture(s)")
        products = {}
        ...
        for index, fixture_path in enumerate(fixture_paths, start=1):
            ...
            product_name = self.product_name(product_type, index)
            attrs = self.load_attrs(fixture_path, product_name, product_type)
            output_key = f"mock_output_{index}"
            logger.info(f"Created {output_key} as {product_type} using fixture {fixture_path}")
            products[output_key] = EOProduct(product_name, attrs=attrs, product_type=product_type)
    ...
    @classmethod
    def output_product_types(cls, output_products: Any | None = None) -> list[str]:
    ...
    @classmethod
    def load_attrs(cls, fixture_path: Path, product_name: str, product_type: str) -> dict:
        """Load fixture metadata and adapt it to the generated mock product."""
        try:
            attrs = cls.read_attrs(fixture_path)
            ...
            attrs = cls.load_default_attrs()

        return cls.normalize_attrs(attrs, product_name, product_type)
    ...
    def normalize_attrs(attrs: dict, product_name: str, product_type: str) -> dict:
        ...
        if not isinstance(stac_discovery.get("geometry"), dict):
            ...
            default_stac = single_unit_mockup.load_default_attrs()["stac_discovery"]
