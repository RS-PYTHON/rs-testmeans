...
def clean_token_dict(config_auth_dict: dict[list], auth_path: str):
    ...
    for key, value in EMPTY_TOKEN_DICT.items():
        ...
        if not isinstance(config_auth_dict[key], value):
            raise TypeError(
                f"""Value from key {key} doesn't have the right type:"""
    ...
    for i in range(len(config_auth_dict["access_token_list"])):
        if (
            current_time - datetime.fromisoformat(config_auth_dict["access_token_creation_date"][i])
        ).total_seconds() >= config_auth_dict["expires_in_list"][i] and (
            (current_time - datetime.fromisoformat(config_auth_dict["refresh_token_creation_date"][i])).total_seconds()
            >= config_auth_dict["refresh_expires_in_list"][i]
        ):
    ...
    if index_to_delete:
        ...
        for key in KEYS_TO_UPDATE:
            config_auth_dict[key] = [
                value for index, value in enumerate(config_auth_dict[key]) if index not in index_to_delete
            ]
...
def token_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if (
            datetime.now() - datetime.fromisoformat(config_auth["access_token_creation_date"][token_index])
        ).total_seconds() >= config_auth["expires_in_list"][token_index]:
            return Response(
                status=HTTPStatus.UNAUTHORIZED,
                response=json.dumps({"message": "Token is valid but is expired!"}),
            )
        ...
        with LOCK:
            ...
            if not refresh_token:
                if username != config_auth["username"] or password != config_auth["password"]:
                    ...
                    return Response(
                        status=HTTPStatus.UNAUTHORIZED,
                        response=json.dumps({"error": "Invalid username and/or password"}),
                    )
