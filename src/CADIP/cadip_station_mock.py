...
def additional_options(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if "$orderby" in display_headers:
            ...
            json_data = sort_responses_by_field(
                json_data,
                field,
                reverse=(ordering_type == "desc"),
            )
        # ICD extract:
...
def batch_response_odata_v4(resp_body: list | map) -> Any:
    """Docstring to be added."""
    unpacked = (
        list(resp_body) if resp_body and not isinstance(resp_body, list) else resp_body
    )
    try:
        data = json.dumps(
            dict(value=unpacked),
        )  # if len(unpacked) > 1 else json.dumps(unpacked[0] if unpacked else [])
    except IndexError:
...
def query_session() -> Response | list[Any]:
    ...
    catalog_path_files = (
        app.config["configuration_path"] / "Catalogue/FileResponse.json"
    )
    ...
    if len(
        ...
        responses = [
            process_session_request(req, request.args, catalog_data)
            for req in split_request
        ]
        ...
        try:
            responses_json = [
                json.loads(resp.data).get("value", json.loads(resp.data))
                for resp in responses
            ]
            responses_norm = [
                resp if isinstance(resp, list) else [resp] for resp in responses_json
            ]
            resp_set = [{d.get("Id") for d in resp} for resp in responses_norm]
            common_response = set.intersection(*resp_set)
            common_elements = [
                d for d in responses_norm[0] if d.get("Id") in common_response
            ]
            # 200 HTTP_OK even if search is empty
            if app.config.get("expand", None) and request.args.get("$expand", None) in [
                "Files",
                "files",
            ]:
            ...
            AttributeError,
        ):  # if a response is empty, whole querry is empty
            return Response(status=HTTPStatus.OK, response=json.dumps({"value": []}))
    elif len(
        split_request := [req.strip() for req in request.args["$filter"].split("or")],
    ) in [2, 3, 4]:
        ...
        common_elements = [
            d for d in sum(responses_norm, []) if d.get("Id") in union_response
        ]
        if app.config.get("expand", None) and request.args.get("$expand", None) in [
            "Files",
            "files",
    ...
    if app.config.get("expand", None) and request.args.get("$expand", None) in [
        ...
        session_response = (
            raw_result["value"] if "value" in raw_result else [raw_result]
        )
        session_response = (
            [] if session_response in [[], [[]]] else session_response
        )  # flatten empty if needed
        for session in session_response:
            ...
            session.update({"Files": [file for file in files]})
        session_response = (
            batch_response_odata_v4(session_response)
            if session_response
            else json.dumps([])
        )
        return Response(
...
def manage_int_querry(op, value, catalog_data, field, headers):
    ...
    return (
        Response(
            status=HTTPStatus.OK,
            response=batch_response_odata_v4(query_result),
            headers=headers,
        )
        if query_result
        else Response(status=HTTPStatus.OK, response=json.dumps({"value": []}))
...
def manage_bool_querry(op, value, catalog_data, field, headers):
    ...
    query_result = [
        product
        for product in catalog_data["Data"]
        if value.lower() == str(product[field]).lower()
    ]
    return (
        Response(
            status=HTTPStatus.OK,
        ...
        case "in":
            ...
            query_result = [
                [
                    product
                    for product in catalog_data["Data"]
                    if product[field] == sat_sid.strip()
                ]
    ...
    match op:
        case "eq":
            ...
            query_result = [
                product
                for product in catalog_data["Data"]
                if norm_value == product[field]
            ]
        case "in":
            values = [normalize_value(v) for v in value.split(",") if v.strip()]
            query_result = [
                product for product in catalog_data["Data"] if product[field] in values
            ]
    return (
        Response(
            status=HTTPStatus.OK,
...
def manage_datetime_querry(op, value, catalog_data, field, headers):
    ...
    match op:
        ...
        case "ge":
            resp_body = [
                ...
                for product in catalog_data["Data"]
                if date < datetime.fromisoformat(product[field])
                or date == datetime.fromisoformat(product[field])
            ]
        case "le":
            resp_body = [
                product
    ...
    return (
        Response(
            status=HTTPStatus.OK,
            response=batch_response_odata_v4(resp_body),
            headers=headers,
        )
        if resp_body
        else Response(status=HTTPStatus.OK, response=json.dumps({"value": []}))
...
def process_session_request(
    ...
    if match := re.search(r"\(([^()]*\sor\s[^()]*)\)", request):
        ...
        responses = [
            process_session_request(cond, headers, catalog_data) for cond in conditions
        ]
        ...
        union_elements = [
            d for d in first_response + second_response if d.get("Id") in union_set
        ]
        return Response(
            status=HTTPStatus.OK,
...
def query_files() -> Response | list[Any]:
    ...
    if match := re.search(r"\(([^()]*\sor\s[^()]*)\)", request.args["$filter"]):
        if request.args["$filter"].split(" and ") == 1:
            conditions = re.split(r"\s+or\s+|\s+OR\s+", match.group(1))
            responses = [
                process_files_request(cond, request.args, catalog_data)
            ...
            for op in request.args["$filter"].split(" and "):
                ...
                union_elements.append(
                    [
                        d
                        for d in first_response + second_response
                        if d.get("Id") in union_set
                    ],
                )
            ...
            common_elements = [
                d for d in first_response if d.get("Id") in common_response
            ]
            return Response(
                status=HTTPStatus.OK,
                response=batch_response_odata_v4(common_elements),
    ...
    if any(header in request.args["$filter"] for header in accepted_operators):
        ...
        if groups:
            first_request, operator, second_request = (
                groups.group(1),
                groups.group(2),
                groups.group(3),
            )
        # split and processes the requests
        first_response = process_files_request(
            first_request.replace('"', ""),
            request.args,
            catalog_data,
        )
        second_response = process_files_request(
        ...
        try:
            first_response = json.loads(first_response.data).get(
                "value",
                json.loads(first_response.data),
            )
        except json.JSONDecodeError:
            first_response = []
        try:
            second_response = json.loads(second_response.data).get(
                "value",
                json.loads(second_response.data),
            )
        except json.JSONDecodeError:
            second_response = []
        # Normalize responses, must be a list, even with one element, for iterator
        first_response = (
            first_response if isinstance(first_response, list) else [first_response]
        )
        second_response = (
            second_response if isinstance(second_response, list) else [second_response]
        )
...
def process_files_request(request, headers, catalog_data):
    ...
    if request.strip().startswith("Id "):
        ...
        if op == "eq":
            ...
            resp_body = [
                product for product in catalog_data["Data"] if product["Id"] == value
            ]
            return Response(
                status=HTTPStatus.OK,
                response=batch_response_odata_v4(resp_body),
        ...
        if op == "in":
            ...
            resp_body = [
                product for product in catalog_data["Data"] if product["Id"] in values
            ]
            return Response(
                status=HTTPStatus.OK,
                response=batch_response_odata_v4(resp_body),
        ...
        match op:
            case "contains":
                resp_body = [
                    product
                    for product in catalog_data["Data"]
                    if filter_value in product[filter_by]
                ]
            case "startswith":
                resp_body = [
                    product
                    for product in catalog_data["Data"]
                    if product[filter_by].startswith(filter_value)
                ]
            case "endswith":
                resp_body = [
                    product
                    for product in catalog_data["Data"]
                    if product[filter_by].endswith(filter_value)
                ]
        return (
            Response(
                status=HTTPStatus.OK,
        ...
        match op:
            case "eq":
                matching = [
                    product
                    for product in catalog_data["Data"]
                    if value[0].strip("('),") == product[field]
                ]
            case "in":
                matching = []
                for idx_list in value:
                    for idx in idx_list.split(","):
                        matching += [
                            product
                            for product in catalog_data["Data"]
                            if idx.strip("(')") in product[field]
                        ]
        return (
            Response(
                response=batch_response_odata_v4(matching),
                status=HTTPStatus.OK,
                headers=headers,
            )
            if matching
...
def redirection(Id) -> Response | list[Any]:
    ...
    if port_number == app.config.get("redirection_port", 1000) and app.config.get(
        "redirection_href",
        None,
    ):
    ...
    if len(files) == 1:
        ...
        if "S3_path" in file_info:
            try:
                ...
                if not (
                    s3_credentials := dotenv.dotenv_values(
            ...
            handler.get_keys_from_s3(
                GetKeysFromS3Config([parts[1]], parts[0], "/tmp/cadip"),
            )
...
def quality_info(Id) -> Response | list[Any]:  # noqa: N803
    ...
    if "expand" in request.args:
        if request.args["expand"] == "qualityInfo":
            catalog_path = (
                app.config["configuration_path"] / "Catalogue/QualityInfoResponse.json"
            )
            catalog_data = json.loads(open(catalog_path).read())
            QIData = map(  # noqa: N806
                ...
                [
                    QIData
                    for QIData in catalog_data["Data"]
...
if (__name__ == "__main__") or gunicorn:
    ...
    if is_expanded := str(os.getenv("CADIP_SESSION_EXPAND", True)).lower() in (
        "true",
        "1",
        "t",
        "y",
    ...
    if not gunicorn:
        if os.getenv("HTTP_REDIRECTION_HREF", None) and args.redirection_port:
            ...
            port_download = multiprocessing.Process(
                target=run_app_on_port,
